            ...
            <form wire:submit="save" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        @if ($profile_image)
                            <div class="mt-4 rounded-sm border border-zinc-700 p-2">
                                <img src="{{ $profile_image->temporaryUrl() }}" alt="{{ __('Profile preview') }}"
                                    class="h-40 w-full object-cover">
                            </div>
                        @elseif($character->profile_image_path)
                            <div class="mt-4 rounded-sm border border-zinc-700 p-2">
                                <img src="{{ Storage::url($character->profile_image_path) }}"
                                    alt="{{ __('Current profile image') }}" class="h-40 w-full object-cover">
                            </div>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        @if ($full_body_image)
                            <div class="mt-4 rounded-sm border border-zinc-700 p-2">
                                <img src="{{ $full_body_image->temporaryUrl() }}" alt="{{ __('Full body preview') }}"
                                    class="h-40 w-full object-cover">
                            </div>
                        @elseif($character->full_body_image_path)
                            <div class="mt-4 rounded-sm border border-zinc-700 p-2">
                                <img src="{{ Storage::url($character->full_body_image_path) }}"
                                    alt="{{ __('Current full body image') }}" class="h-40 w-full object-cover">
                            </div>
                        @endif
                    </div>
                </div>
