<div class="py-10">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <p class="text-sm uppercase tracking-[0.3em] text-zinc-500">CMS — Badges</p>
                <h1 class="text-4xl font-bold uppercase tracking-wider">Badge Types</h1>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.badges') }}" class="inline-flex items-center border border-zinc-700 text-zinc-400 px-4 py-2 text-sm font-semibold tracking-wider uppercase transition hover:text-white hover:border-zinc-500">
                    Back
                </a>
        ...
        <div class="rounded-sm bg-zinc-900 border border-zinc-800 overflow-hidden">
            <div class="px-4 py-4 border-b border-zinc-800">
                ...
                <h2 class="text-lg font-semibold uppercase tracking-wider">All Types</h2>
            </div>
                    ...
                    <tbody class="divide-y divide-zinc-800">
                        @foreach ($types as $type)
                            <tr class="hover:bg-zinc-800/50 transition">
                                <td class="px-4 py-4 text-white font-medium">{{ $type->name }}</td>
                                <td class="px-4 py-4"><code class="text-xs text-zinc-500 font-mono">{{ $type->slug }}</code></td>
                                <td class="px-4 py-4 text-center text-zinc-400 text-sm">{{ $type->badges_count }}</td>
                                <td class="px-4 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button wire:click="delete({{ $type->id }})"
                                            wire:confirm="Delete this type? Badges using it will have no type."
        ...
        >
            ...
            <div class="relative bg-zinc-900 border border-zinc-800 w-full max-w-md" @click.stop>
                <div class="sticky top-0 z-10 bg-zinc-800 text-accent px-5 py-3 text-sm font-semibold uppercase tracking-wider flex items-center justify-between">
                    <span>{{ $editingId ? 'Edit Type' : 'New Type' }}</span>
                    ...
                    <form wire:submit="save">
                        <div class="mb-4">
                            ...
                            <input type="text" wire:model="name"
                                ...
                                placeholder="e.g. Character, Event, Location">
